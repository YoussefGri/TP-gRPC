...
import com.example.agencehotelgrpc.dto.ReservationDTO;
import com.example.agencehotelgrpc.exceptions.BadDatesException;
import com.example.agencehotelgrpc.exceptions.NoRoomsFoundException;
...
public class ReservationService {
    ...
    public ReservationDTO effectuerReservation(int idOffre,
        ...
        ReservationDTO reservationDTO = new ReservationDTO(reservationId, offre.getHotel().getNom(), offre.getHotel().getAdresse().getVille(), dateDebut, dateFin, nomClient, prenomClient, emailClient, prix);
    ...
    private List<Chambre> selectChambresForReservation(int hotelId, int offreId, Date dateCheckin, Date dateCheckout, int nbPersonnes)
        ...
        System.err.println("parametres recus : " + hotelId + " " + offreId + " " + dateCheckin + " " + dateCheckout + " " + nbPersonnes);

        Offre offre = offreRepository.findById(offreId)
                .orElseThrow(() -> new OffreNotFoundException("Offre inexistante"));

        hotelId = offre.getHotel().getId();
        ...
        chambresDisponibles.sort((c1, c2) -> Integer.compare(c1.getNbPersonnes(), c2.getNbPersonnes()));

        for (Chambre chambre : chambresDisponibles) {
            if (personnesRestantes <= 0) break;

            // Sélectionner la chambre même si elle dépasse la capacité demandée
            if (chambre.getNbPersonnes() >= personnesRestantes) {
                chambresAReserver.add(chambre);
                personnesRestantes = 0; // Réservation satisfaite
            } else {
                chambresAReserver.add(chambre);
